...
daily_pageviews_bynews <- rbind(readr::read_rds("data/daily_pageviews_bynews.rds")) %>%
  mutate(date = lubridate::ymd(date)) %>%
  filter(date >= as.Date("2018-03-26")) #Filter out earlier dates to help increase clarity on values around launch date
...
daily_pageviews_bynews$url <- sub(".*(Associated_Press).*", "Associated Press", daily_pageviews_bynews$url)

p <- ggplot(daily_pageviews_bynews, aes(x = date, y = requests, fill = url)) +
  geom_bar(position = "dodge", stat = "identity") +
  geom_vline(xintercept = as.numeric(as.Date("2018-04-03")),
             linetype = "dashed", color = "blue") +
  geom_text(aes(x=as.Date('2018-04-03'), y=200, label="Facebook Article Context Feature Launch Date"), size=3, vjust = -1.2, angle = 90, color = "black") +
  scale_y_continuous("pageviews (bots excluded)", labels = polloi::compress) +
  scale_x_date(labels = date_format("%d-%b-%y"), date_breaks = "1 day") +
  scale_fill_brewer("News Source", palette = "Set3") +
  ggthemes::theme_tufte(base_size = 12, base_family = "Gill Sans") +
  theme(axis.text.x=element_text(angle = 45, hjust = 1),
        panel.grid = element_line("gray70")) +
...
pv_avg_prior <- daily_pageviews_bynews %>%
  ...
  summarise(mean = mean(requests))

pv_avg_after <- daily_pageviews_bynews %>%
  filter(date >= "2018-04-03" & date <= "2018-04-08") %>%
  summarise(mean = mean(requests))
